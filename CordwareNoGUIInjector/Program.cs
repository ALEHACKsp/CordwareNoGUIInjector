...
namespace CordwareNoGUIInjector
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ConsoleUtils.Log("Do you want to inject or eject Cordware? (inject for injection, eject for ejection)");
                string injectoreject = Console.ReadLine();

                if (injectoreject.ToLower() == "inject")
                {
                ...
                {
                    ConsoleUtils.Log("What client do you use? (Or want to eject from) (Type discord for Discord, discordcanary for Canary, etc): ");
                    string client = Console.ReadLine().ToLower();
                    string appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
                    var folders = Directory.GetDirectories($"{appData}\\{client}");
                    var versionfolderpath = folders[0];
                    ...
                    if (!Directory.Exists($"{versionfolderpath}\\modules\\discord_desktop_core\\Cordware"))
                        ConsoleUtils.LogError("Cordware isn't injected!");
                    else
                    {
                        File.WriteAllText($"{versionfolderpath}\\modules\\discord_desktop_core\\index.js", "module.exports = require('./core.asar');");
                        Directory.Delete($"{versionfolderpath}\\modules\\discord_desktop_core\\Cordware");
                        ConsoleUtils.Log($"Ejected successfully!");
                    }
                }
            }
            catch(Exception e) {
